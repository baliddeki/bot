...
class StrategyConfig:
    ...
    fvg_tfs: List[Timeframe] = field(
        default_factory=lambda: ["M3", "M5", "M15", "M30", "H1"]
    )
    ...
    sl_pips: float = 90.0  # Fixed SL distance in pips
...
class RiskConfig:
    ...
    small_risk_pct: float = 0.05  # 6% risk per trade
    small_max_daily_loss_pct: float = 0.1  # 6% max daily loss

    # ── Large account (balance ≥ big_acct_min_balance) ──
    big_acct_min_balance: float = 10000.0
    big_risk_pct: float = 0.01  # 1% risk per trade
    big_max_daily_loss_pct: float = 0.03  # 4% max daily loss
